...
- `lib/db.ts` and `lib/linkRepository.ts`
  Persist links, metadata, statuses, and errors in SQLite.
- `app/page.tsx` and `components/*`
...
The provided mock metadata service is expected to run separately from the sibling `mock-service` directory in the workspace:
